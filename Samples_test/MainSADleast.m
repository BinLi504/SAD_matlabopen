...
i=1;
%save cost value
f_itn_final = NaN(2000,1); % save cost at each iteration 
cost_final = NaN(2000,1); % save cost at each iteration
cost_norm1 = NaN(2000,1); % save cost at each iteration
cost_norm2 = NaN(2000,1); % save cost at each iteration.
...
Ym = mean(Y);
 ...
 while(i2 <=1) 
[Xdn,cost] = SAD_least(Y, Omega,reliable_samples_mat,X_sparse,ClippingLevel,clipped_pos_mat,clipped_neg_mat,ZZ);
i2=i2+1;
end;
temp22=cost;   
